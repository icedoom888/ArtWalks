...
def save_originals(images, save_path, idx, same_ratio=False, square_crop=False, target_img_size=None):
...
def image_generation(args):
    ...
    for idx, (img1_path, img2_path, og_img1_path, og_img2_path) in tqdm(enumerate(zip(artworks, artworks[1:], og_artworks, og_artworks[1:])), total=len(artworks)):
        ...
        if not args.no_originals:
            ...
            save_originals(images, interpolation_save_path, idx, same_ratio=same_ratio, square_crop=args.square_crop, target_img_size=(args.width, args.height))
